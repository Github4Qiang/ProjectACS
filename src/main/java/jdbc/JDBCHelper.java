...
public class JDBCHelper implements Serializable {
     ...
     */
    private JDBCHelper() {
        int datasourceSize = ConfigurationManager.getInteger(Constants.CONN_POOL_SIZE);
        //创建指定连接数量的数据库连接池
    ...
    public boolean executeInsert(String sql, Object[] params) {
        Connection conn = null;
        PreparedStatement pstmt = null;
        int result = 0;
        try {
            ...
            System.out.println("Duplicate Insert" + params);
            if (params != null && params.length > 0) {
                for (int i = 0; i < params.length; i++) {
